...
test('conversation navigation can verify an exact provider route with chat header/composer and a closed profile panel',()=>{
 const h=load('<header><h2>Test Customer</h2></header><div role="textbox" contenteditable="true" aria-label="Reply"></div>');
 const r=h.w.TenhFacebookProfileResolver.readConversation(opts);assert.equal(r.found,true);assert.equal(r.navigationIdentity,'header:test customer');assert.equal(h.read().profileUrl,undefined);h.close();
});
test('header-only navigation cannot authorize a redirected legacy link or a different customer',()=>{
 for(const [name,options] of [['Other Customer',opts],['Test Customer',{...opts,conversationLink:'https://www.facebook.com/123456/inbox/888888/?section=messages',loadedConversationLink:base}]]){
  const h=load(`<header><h2>${name}</h2></header><div role="textbox" contenteditable="true" aria-label="Reply"></div>`);
  assert.notEqual(h.w.TenhFacebookProfileResolver.readConversation(options).found,true);h.close();
 }
});
test('matching name in chat content, ambiguous headers or missing conversation panel cannot authorize navigation',()=>{
 for(const html of ['<div role="log"><header><h2>Test Customer</h2></header></div><div role="textbox" contenteditable="true"></div>',
 '<header><h2>Test Customer</h2><h2>Another Customer</h2></header><div role="textbox" contenteditable="true"></div>',
 '<header><h2>Test Customer</h2></header>']){
