...
function worker(){
 ...
 const chrome={runtime:{id:'test',getManifest:()=>({version:'1.2.30'}),onMessage:event('message'),onConnect:event('connect'),onInstalled:event('install'),onStartup:event('startup')},storage:{local:storage,session:storage},tabs:{onUpdated:event('tabUpdate'),onRemoved:event('tabRemove')},alarms:{onAlarm:event('alarm')}};
...
test('manifest injects the TENH bridge on exactly the requested local server',()=>{
 const m=JSON.parse(fs.readFileSync(root+'/manifest.json'));assert.equal(m.version,'1.2.30');assert.ok(m.host_permissions.includes('http://localhost:3000/*'));
