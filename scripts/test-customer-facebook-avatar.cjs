...
function harness(savedId){
  ...
  const context={exports,window:{open:(...args)=>windows.push(args)},localStorage:{getItem:key=>cache.get(key),setItem:(key,value)=>cache.set(key,value)},
    require:name=>{
      if(name==='react') return {useState:initial=>[initial,()=>{}],useRef:initial=>({current:initial}),useEffect:fn=>cleanups.push(fn())};
      ...
      if(name==='@/lib/extension/use-companion')return {useCompanion:()=>({installed:true,version:'1.2.15',openFacebookProfile:()=>pending,openResolvedFacebookProfile:async token=>{confirmed.push(token);return {opened:true}}})};
      throw Error(name);
    }};
  vm.runInNewContext(component,context);
  const tree=exports.CustomerFacebookAvatar({conversation:{id:'conversation',business_id:'workspace',social_account:{platform:'facebook',platform_account_id:'123456'},contact:{id:'contact',platform_user_id:'987654',full_name:'Customer',facebook_profile_id:savedId}}});
  return {windows,confirmed,cache,answer,click:()=>tree.props.children[0].props.onClick(),unmount:()=>cleanups.forEach(fn=>fn?.())};
}
