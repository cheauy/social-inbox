...
function harness(config={}){
 ...
 fetch:async(url,init={})=>{
   ...
   if(name==='@/lib/extension/use-companion')return {useCompanion:()=>({installed:config.installed!==false,version:config.version||'1.2.23',
...
test('old or absent extension produces update instruction immediately',async()=>{
 for(const config of [{installed:false},{version:'1.2.22'}]){const h=harness(config);h.click();await tick();assert.equal(h.lookups.length,0);assert.ok(JSON.stringify(h.render()).includes('1.2.23'));}
});
