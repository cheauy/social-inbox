...
function harness(result = {profileUrl:'https://www.facebook.com/thy.thy.886036#',pageId:'393342417206745',selectedItemId:'61555135812581'}, tabs = [{id:77,url:'https://business.facebook.com/latest/inbox/all/?asset_id=393342417206745'}]) {
  ...
  const context=vm.createContext({URL, URLSearchParams, crypto:webcrypto, TENH_ORIGIN:'https://app.tenhchat.com',
    ...
    chrome:{tabs:{query:async()=>tabs,update:async()=>{throw Error("Unexpected tab navigation")},create:async options=>{created.push(options);return {id:created.length+100}}, remove:async id=>removed.push(id)},
...
test('missing profile leaves existing tabs untouched',async()=>{
  ...
  assert.equal(result.reason,'profile_link_missing');
...
test('no open Facebook tabs never causes a Business Suite fallback',async()=>{
  ...
  const result=await context.openFacebookCustomerProfile(options,sender);
  assert.equal(result.reason,'profile_link_missing');assert.equal(created.length,0);
});
