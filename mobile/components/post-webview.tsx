  ...
  return <Modal visible animationType="slide" onRequestClose={back}>
    <View style={{ flex: 1, paddingTop: insets.top, paddingBottom: insets.bottom, backgroundColor: "white" }}>
      <View style={{ flexDirection: "row", alignItems: "center", padding: 8, borderBottomWidth: 1, borderColor: colors.border }}>
        ...
        <Text style={{ flex: 1, fontSize: 17, fontWeight: "600", color: colors.ink }}>Facebook post</Text>
        <Pressable accessibilityLabel="Close post" onPress={onClose} style={{ padding: 12 }}><Ionicons name="close" size={24} color={colors.ink} /></Pressable>
      </View>
      {error ? <Pressable onPress={() => { setError(false); browser.current?.reload(); }} style={{ padding: 16 }}><Text style={{ color: colors.blue }}>Unable to load post. Tap to retry.</Text></Pressable> : null}
