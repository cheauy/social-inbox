...
export const supabase = createClient(url || "https://unconfigured.supabase.co", key || "unconfigured", {
  auth: { storage: sessionStorage, autoRefreshToken: true, persistSession: true, detectSessionInUrl: false, lock: processLock, /* Google and Facebook come back through the app's own scheme with a code to
     exchange; the verifier never leaves this device. */ flowType: "pkce" },
});
