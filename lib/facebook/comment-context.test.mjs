...
function setup({ authorized = true, active = true, missing = false } = {}) {
  ...
  const dependencies = {
    ...
    "@/lib/facebook/get-facebook-page-access-token": { getFacebookPageAccessToken: async (page) => {
      assert.equal(page, "page"); tokens++; return "test-token";
    } },
  };
...
test("denied access stops before loading conversation, Page or token", async () => {
