...
function harness(state, commentId = "post_comment", rejected = false, blockReason = null) {
  const requests = [];
  const records = {
  ...
  const { POST } = load("../../app/api/facebook/send/route.ts", {
    ...
    "@/lib/inbox/message-actions": { createReplyContext() { throw new Error("Unexpected quote in plain-send test"); }, getMessageActions() { throw new Error("Unexpected quote in plain-send test"); } },
    "@/lib/inbox/mutate-message-metadata": { mutateMessageMetadata() { throw new Error("Unexpected quote mutation in plain-send test"); } },
...
test("confirmed Facebook customer block prevents normal text send", async () => {
  const app = harness("standard", "post_comment", false, "Customer blocked");
  assert.equal((await app.send()).status, 403); assert.equal(app.requests.length, 0);
});
