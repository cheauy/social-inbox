          ...
          <div key={section.labelKey}>
            ...
            <div className="space-y-1">
              {visibleItems.map((item) => {
                ...
                return (
                  /*
                   * Matches the Analytics sidebar: a blue-tinted active row
                   * with the icon lifted into a white rounded tile, instead of
                   * the old flat grey highlight.
                   */
                  <Link
                    key={item.href}
                    href={item.href}
                    aria-current={isActive ? "page" : undefined}
                    className={`group flex min-h-9 items-center gap-2 rounded-xl px-2 py-1.5 text-sm transition ${
                      isActive
