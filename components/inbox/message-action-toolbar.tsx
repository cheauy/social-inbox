...
export function MessageActionToolbar({ outgoing, actions, replying, pinned, pinPending, reactionControl, keepVisible, photoHover, onReply, onPin, onEdit, onDelete }: MessageActionToolbarProps) {
  if (!Object.values(actions).some(Boolean) && !reactionControl) return null;
  ...
  return (
    <div className={`mt-1 flex px-1 ${keepVisible ? "opacity-100" : "opacity-0"} transition-opacity duration-150 focus-within:opacity-100 ${photoHover ? "group-hover/photo:opacity-100" : "group-hover:opacity-100"} [@media(hover:none)]:opacity-100 ${outgoing ? "justify-end" : "justify-start"}`}>
