...
export function MessengerMessageActions({ message, platform, hideReaction, onMessagePatched, ...toolbar }: Props) {
  ...
  const queued = useRef<{ emoji: string | null } | null>(null);
  ...
  async function react(emoji: string | null) {
    ...
    if (inFlight.current) {
      ...
      queued.current = { emoji }; setOptimistic(emoji); setOpen(false); return;
    }
    const target = latest.current, targetScope = scope;
    let confirmedSend = false;
    requestScope.current = targetScope;
    ...
    try {
      ...
      queued.current = null;
      if (alive.current && currentScope.current === targetScope) {
        // Coalesce rapid changes; never send concurrently or retry an uncertain result.
        if (confirmedSend && next && next.emoji !== emoji) { void react(next.emoji); }
        else { setPending(false); setOptimistic(undefined); }
      }
    }
  }
  ...
  return <>
    ...
    <MessageActionToolbar {...toolbar} keepVisible={open || pending} reactionControl={allowed ? <button ref={trigger} type="button" aria-label="React to message" title="React to message" aria-haspopup="dialog" aria-expanded={open} onClick={togglePicker} className="touch-manipulation flex h-7 w-8 items-center justify-center rounded-full text-slate-500 hover:bg-blue-50 hover:text-blue-600 focus-visible:outline-2 focus-visible:outline-blue-500 disabled:cursor-wait">
    ...
    {open && allowed && position ? createPortal(<div ref={panel} role="dialog" aria-label="Choose a Messenger reaction" className="fixed z-[10000] w-72 max-w-[calc(100vw-16px)] rounded-2xl border border-slate-200 bg-white p-2 shadow-xl" style={position} onClick={event => event.stopPropagation()}>
      <div className="flex items-center justify-between gap-0.5">
        {MESSENGER_QUICK_REACTIONS.map(item => <button key={item.emoji} type="button" title={item.label} aria-label={`${pageEmoji === item.emoji ? "Remove" : "React with"} ${item.label}`} aria-pressed={pageEmoji === item.emoji} onClick={() => void react(pageEmoji === item.emoji ? null : item.emoji)} className={`touch-manipulation flex h-10 w-9 items-center justify-center rounded-full text-2xl transition hover:scale-110 hover:bg-blue-50 focus-visible:outline-2 focus-visible:outline-blue-500 ${pageEmoji === item.emoji ? "bg-blue-100" : ""}`}>{item.emoji}</button>)}
      </div>
